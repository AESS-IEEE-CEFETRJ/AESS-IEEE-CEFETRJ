...
float altitudeBase = 0;  // Novo: altitude de referência (0 m no início)
...
void setup() {
  ...
  if (bmpAvailable) {
    ...
    // Define altitude inicial como 0
    delay(500);
    altitudeBase = bmp.readAltitude(1013.25);
    Serial.printf("Altitude de referência definida: %.2f m\n", altitudeBase);
  } else {
...
void loop() {
  ...
  // Lê BMP280 e aplica altitude relativa
  if (bmpAvailable) {
    bmpTemp = bmp.readTemperature();
    altitude = bmp.readAltitude(1013.25) - altitudeBase; // Agora 0 é o ponto inicial
  ...
  Serial.printf("Temperatura MPU: %.2f °C, Temperatura BMP: %.2f °C, Altitude Relativa: %.2f m\n",
